...
export const updateOrderStatus = async (req, res) => {
    try {
        ...
        return res.status(500).json({ message: `update Order error ${error}` })
    }
}
...
export const acceptOrder = async (req, res) => {
    try {
        ...
        if (alreadyAssigned) {
            return res.status(400).json({ message: "You are already assigned to an order" })
        }
...
export const getCurrentOrder = async (req, res) => {
    try {
        ...
        return res.status(500).json({ message: `get Current Order Assignment ${error}` })
    }
}
...
export const sendDeliveryOtp = async (req, res) => {
    try {
        ...
        return res.status(200).json({ message: `OTP send succesfully to ${order?.user?.fullName}` })
    } catch (error) {
